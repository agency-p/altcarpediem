<template>
    <v-container>
        <v-card class="mx-auto mt-8" rounded="xl" elevation="4">
            <v-toolbar flat class="rounded-t-xl">
                ...
                <v-toolbar-items>
                    ...
                    <v-select transition="scroll-y-transition" :items="countrys" variant="outlined" density="compact"
                        label="Country" type="text" class="mt-9 mr-2" v-model="user.country">
                    </v-select>
                    <v-select transition="scroll-y-transition" :items="['Months', 'Weeks', 'Days']" variant="outlined"
                        density="compact" label="Mode" type="text" class="mt-9" v-model="mode"></v-select>
                </v-toolbar-items>
            </v-toolbar>

            <v-card-text class="pa-8">
                <v-row id="squares" class="squares"></v-row>
...
export default {
    ...
    computed: {
        renderCarpediem() {
            ...
            let children = [].slice.call(document.getElementById("squares").getElementsByTagName("div"), 0)
            ...
            for (let i = 1; i < itemCounter; i++) {
                ...
                squares.insertAdjacentHTML('beforeend', `<div id="square" class="square" data-level="${level}"></div>`);
            }
        },
